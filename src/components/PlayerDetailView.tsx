...
export function PlayerDetailView({ playerId, sport, selectedProps, onBack }: PlayerDetailViewProps) {
  ...
  const fetchPlayer = async () => {
    ...
    try {
      ...
      setGeneratedLines(Array.isArray(data.generated_lines) ? data.generated_lines : []);
      
      const generatedProp = (Array.isArray(data.generated_lines) ? data.generated_lines : []).find((l: any) => 
        typeof l?.prop === "string" && l.prop === (selectedProps[0] || "").toUpperCase()
      );
      if (generatedProp && typeof generatedProp.line === "number") {
        setPlayerLine(generatedProp.line);
      } else {
        const pAvg = calcAvgForProp(data.player.stats, 10, selectedProps[0] || "points");
  ...
  const getPropValue = (game: any, propId: string) => {
    ...
    if (!group) return 0;
    if (group.stackKeys && Array.isArray(group.stackKeys)) {
      return group.stackKeys.reduce((sum, key) => {
        const val = game[key];
            ...
            return (
              <div key={i} className="flex-1 flex flex-col items-center group relative h-full">
                <div className="opacity-0 group-hover:opacity-100 absolute -top-10 bg-[#0f172a] border border-gray-700 text-xs p-2 rounded z-30 pointer-events-none whitespace-nowrap transition-opacity">
                  <p className="text-yellow-400 font-bold">{typeof g.opponent === "string" ? g.opponent : "??"} ({typeof g.game_date === "string" ? g.game_date : ""})</p>
                ...
                <div className="absolute -bottom-6 w-full text-center">
                  <p className="text-[10px] text-gray-500 font-bold truncate">{typeof g.opponent === "string" ? g.opponent : "??"}</p>
                </div>
              </div>
            );
          })}
  ...
  const currentGeneratedLine = Array.isArray(generatedLines) ? generatedLines.find((l: any) => 
    typeof l?.prop === "string" && l.prop === selectedPlayerProp.toUpperCase()
  ) : undefined;
      ...
      <div className="p-4 max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <h1 className="text-2xl font-bold text-yellow-400">{typeof player.full_name === "string" ? player.full_name : "Unknown"}</h1>
            <p className="text-sm text-gray-400">{typeof player.team === "string" ? player.team : ""} • {typeof player.position === "string" ? player.position : ""}</p>
          </div>
        </div>
          ...
          {currentGeneratedLine && (
            <div className="flex flex-wrap items-center gap-4 mb-4 p-3 bg-[#1e293b] rounded-lg border border-gray-700">
              <div className="text-sm">
                ...
                <span className="ml-2 font-bold text-yellow-400">{typeof currentGeneratedLine.line === "number" ? currentGeneratedLine.line.toFixed(1) : "-"}</span>
              </div>
              <div className="text-sm">
                ...
                {games.slice(0, 15).map((g: any, i: number) => {
                  ...
                  return (
                    <tr key={i} className={`hover:bg-[#1e293b] transition ${isOver ? "bg-green-900/10" : "bg-red-900/10"}`}>
                      <td className="p-3 font-medium text-gray-200 whitespace-nowrap">{typeof g.opponent === "string" ? g.opponent : "??"}</td>
                      <td className="p-3 text-gray-400 whitespace-nowrap">{typeof g.game_date === "string" ? g.game_date : ""}</td>

...
// ✅ FIX: Correct Edge Function endpoint
...
export default function Admin() {
  ...
  const syncSport = async (sport: string) => {
    ...
    try {
      const res = await fetch(EDGE_URL, {
        ...
        body: JSON.stringify({ 
          // ✅ FIX: Use correct operation name
          operation: "sync_sport",
          sport 
        }),
      });
  ...
  // ✅ AUTO SYNC (every 30 mins) - Optional, can be disabled
  useEffect(() => {
    if (!isAdmin) return;
    // Uncomment below to enable auto-sync
    // const run = async () => { await syncAll(); };
    // run();

...
import { RefreshCw, Trophy, TrendingUp, ExternalLink, ChevronDown, ChevronUp } from "lucide-react";
...
// 🔥 FIXED: Use today's date without timezone offset
...
export default function HorseRacing() {
  ...
  const loadPicks = async (r: string) => {
    console.log("🔄 loadPicks called with region:", r);
    ...
    try {
      ...
      console.log("✅ Loaded", d.count, "picks");
      setPicks(d.picks || []);
  ...
  const loadRaceCard = async (pick: Pick) => {
    const key = `${pick.track_name}-${pick.race_time}`;
    if (expandedRace === key) { 
    ...
    try {
      ...
      const d = await res.json();
      // For now, just show all picks as "race card"
      setRaceCard(d.picks || []);
    } catch(err) {
      ...
      <div className="p-4 max-w-6xl mx-auto">
        ...
        {!loading && picks.length === 0 && (
          <div className="text-center py-20 border border-gray-800 rounded-xl bg-gray-900/20">
            ...
            <p className="text-gray-600 text-sm mt-1">Date: {today} · Region: {region}</p>
            <button onClick={syncCards} disabled={syncing}
          ...
          <div key={`${r}-${track}`} className="mb-4 bg-gray-900/30 border border-gray-800 rounded-xl overflow-hidden">
            ...
            <div className="px-4 py-3 border-b border-gray-800 flex items-center justify-between bg-gray-900/50">
              ...
              <span className={`text-xs px-2 py-0.5 rounded-full border font-semibold uppercase ${
                r === 'aus' ? 'border-green-500/40 text-green-400' : 'border-red-500/40 text-red-400'
              }`}>{r.toUpperCase()}</span>
            </div>
            ...
            {trackPicks.map(pick => {
              ...
              return (
                <div key={pick.id || pick.horse_name} className="border-b border-gray-800/50 last:border-0">
                  ...
                  {isExpanded && (
                    <div className="bg-gray-900/50 border-t border-gray-800/50 px-4 py-3">
                      ...
                      {loadingCard ? (
                        ...
                        <div className="text-center py-4 text-gray-600 text-sm">No data</div>
                      ) : (
                        <div className="space-y-2">
                          {raceCard.map((runner, idx) => (
                            <div key={idx} className="flex items-center justify-between py-1.5 px-2 rounded-lg bg-gray-800/30">
                              <div className="flex items-center gap-2">
                              ...
                              <span className="text-xs text-gray-500">{Math.round((runner.win_rate||0)*100)}% win</span>
                            </div>
                          ))}
                        </div>
                      )}

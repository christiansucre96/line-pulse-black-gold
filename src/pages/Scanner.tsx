...
// Working version – table, pagination, game filter, no PlayerCard, no rollingStats
...
export default function Scanner() {
  ...
  const fetchGames = async () => {
    try {
      ...
      else console.error("Failed to fetch games:", data.error);
    } catch (err) {
  ...
  const fetchPlayers = async (page: number = 1, gameId?: string) => {
    ...
    try {
      const body: any = { operation: "get_players", sport, page, limit: PLAYERS_PER_PAGE };
  ...
  const displayPlayers = useMemo(() => {
    ...
    list.sort((a, b) => {
      const getVal = (p: any) => {
        ...
        if (sortKey === "diff") return (pd.avg_l10 ?? 0) - (pd.line ?? 0);
        return 0;
      };
  ...
  const SortTh = ({ label, sk }: { label: string; sk: string }) => (
    <th onClick={() => handleSort(sk)} className="p-3 text-left text-[11px] font-semibold text-yellow-400/70 uppercase tracking-wider cursor-pointer select-none hover:text-yellow-400 whitespace-nowrap">
      ...
      <div className="p-4 max-w-7xl mx-auto">
        <div className="mb-5 flex items-start justify-between">
          ...
          <div className="flex gap-2">
            <button onClick={() => fetchPlayers(currentPage, selectedGame === "all" ? undefined : selectedGame)} disabled={loading} className="flex items-center gap-2 px-3 py-1.5 bg-gray-800 hover:bg-gray-700 rounded-lg text-sm text-gray-300 transition disabled:opacity-50">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          <Select value={selectedGame} onValueChange={handleGameChange}>
            <SelectTrigger className="w-72 bg-gray-900 border-gray-700 text-gray-300 text-sm"><SelectValue placeholder="All Games (Next 24h)" /></SelectTrigger>
            ...
            <div className="bg-gray-900/30 border border-gray-800 rounded-xl overflow-hidden">
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {displayPlayers.map((p, i) => {
                      ...
                      return (
                        <tr key={`${p.player_id}-${i}`} onClick={() => navigate(`/scanner?playerId=${p.player_id}&sport=${sport}`)} className="border-b border-gray-800/50 hover:bg-gray-800/40 cursor-pointer transition">
                          <td className="p-3">
                            <div className="flex items-center gap-2.5">
                              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-yellow-500 to-yellow-700 flex items-center justify-center text-black text-xs font-bold shrink-0">{getInitials(p.name)}</div>
                              <div>
                          ...
                          <td className="p-3">
                            <div><p className="text-xs text-gray-400">vs {p.opponent}</p><p className="text-[10px] text-gray-600">{p.game_date}</p></div>
                          </td>
                          ...
                          <td className="p-3"><span className={`text-sm font-semibold ${diff > 0 ? "text-green-400" : diff < 0 ? "text-red-400" : "text-gray-500"}`}>{diff > 0 ? "+" : ""}{diff.toFixed(1)}</span></td>
                          ...
                          <td className="p-3"><span className={`text-xs font-bold ${pd.trend === 'up' ? 'text-green-400' : pd.trend === 'down' ? 'text-red-400' : 'text-gray-600'}`}>{pd.trend === 'up' ? '↑ HOT' : pd.trend === 'down' ? '↓ COLD' : '→ FLAT'}</span></td>
                        </tr>
                      );
                    })}
                  </tbody>
              ...
              {totalPages > 1 && (
                <div className="flex justify-between items-center px-4 py-3 border-t border-gray-800 bg-gray-900/30">
                  <button onClick={() => fetchPlayers(Math.max(1, currentPage - 1), selectedGame === "all" ? undefined : selectedGame)} disabled={currentPage === 1 || loading} className="flex items-center gap-1 px-3 py-1.5 bg-gray-800 hover:bg-gray-700 rounded text-sm disabled:opacity-50 disabled:cursor-not-allowed transition"><ChevronLeft className="w-4 h-4" /> Prev</button>
                  <span className="text-sm text-gray-400">Page {currentPage} of {totalPages} ({totalPlayers} total)</span>
                  <button onClick={() => fetchPlayers(Math.min(totalPages, currentPage + 1), selectedGame === "all" ? undefined : selectedGame)} disabled={currentPage >= totalPages || loading} className="flex items-center gap-1 px-3 py-1.5 bg-gray-800 hover:bg-gray-700 rounded text-sm disabled:opacity-50 disabled:cursor-not-allowed transition">Next <ChevronRight className="w-4 h-4" /></button>
                </div>
              )}
            </div>
          )

...
function formatTimeET(utc: string): string {
  if (!utc) return "";
...
function calcRolling(values: number[], todayVal: number | null): { avg: number; count: number } {
  ...
  const avg = Math.round((win.reduce((a, b) => a + b, 0) / win.length) * 10) / 10;
  return { avg, count: win.length };
}
...
export default function Scanner() {
  ...
  const loadGames = useCallback(async (s: string) => {
    ...
    const filtered = (data || []).filter((g: any) => g.home_team?.abbreviation && g.away_team?.abbreviation);
    const options: GameOption[] = filtered.map((g: any) => {
      const home = g.home_team?.abbreviation || "?";
      const away = g.away_team?.abbreviation || "?";
      const suffix = g.status === "finished" ? " ✓" : g.status === "live" ? " 🔴" : "";
      return {
        game_id: g.external_id,
        label: `${away} vs ${home}${suffix}`,
        ...
        <div className="mb-5 flex items-start justify-between">
          <div>
            ...
            <p className="text-gray-500 text-sm mt-0.5 flex items-center gap-1.5">
              <Calendar className="w-3.5 h-3.5" /> Players with games today ({today})
            </p>
          </div>
          <button
